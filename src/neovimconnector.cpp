...
NeovimConnector::NeovimConnector(MsgpackIODevice *dev) : QObject(), m_dev(dev) {
    m_helper = new NeovimConnectorHelper(this);
    qRegisterMetaType<NeovimError>("NeovimError");
    qRegisterMetaType<int64_t>("int64_t");

    connect(m_dev, &MsgpackIODevice::error, this,
            &NeovimConnector::msgpackError);
...
void NeovimConnector::setRequestTimeout(int ms) { this->m_timeout = ms; }
 ...
 */
NeovimConnector::NeovimError NeovimConnector::errorCause() { return m_error; }

/**
 * An human readable error message for the last error
 */
QString NeovimConnector::errorString() { return m_errorString; }
 ...
 */
uint64_t NeovimConnector::channel() { return m_channel; }
...
void NeovimConnector::discoverMetadata() {
    ...
    connect(r, &MsgpackRequest::finished, m_helper,
            &NeovimConnectorHelper::handleMetadata);
    connect(r, &MsgpackRequest::error, m_helper,
            &NeovimConnectorHelper::handleMetadataError);
 ...
 * @see ready
 */
bool NeovimConnector::isReady() { return m_ready; }
...
NeovimApi1 *NeovimConnector::neovimObject() { return api1(); }
...
NeovimConnector *NeovimConnector::spawn(const QStringList &params,
    ...
    connect(p, SIGNAL(finished(int, QProcess::ExitStatus)), c,
...
NeovimConnector *NeovimConnector::connectToSocket(const QString &path) {
    ...
    connect(s, SIGNAL(error(QLocalSocket::LocalSocketError)), c,
            SLOT(socketError()));
...
NeovimConnector *NeovimConnector::connectToHost(const QString &host, int port) {
    ...
    connect(s, SIGNAL(error(QAbstractSocket::SocketError)), c,
            SLOT(socketError()));
    connect(s, &QAbstractSocket::connected, c,
            &NeovimConnector::discoverMetadata);
...
/**
 * True if NeovimConnector::reconnect can be called to reconnect with Neovim.
 * This is true unless you built the NeovimConnector ctor directly instead of
 * using on of the static methods.
 */
bool NeovimConnector::canReconnect() { return m_ctype != OtherConnection; }
...
quint64 NeovimConnector::apiCompatibility() { return m_api_compat; }

/** The maximum API level supported by this instance */
quint64 NeovimConnector::apiLevel() { return m_api_supported; }
 ...
 *
 * If the Neovim process was started using NeovimQt::NeovimConnector::spawn this
 * signal is emitted when the process exits.
 */
