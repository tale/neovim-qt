...
void NeovimConnectorHelper::handleMetadataError(quint32 msgid, quint64,
 ...
 * - Set channel_id
 */
void NeovimConnectorHelper::handleMetadata(quint32 msgid, quint64,
    ...
    if (asList.size() != 2 || !asList.at(0).canConvert<quint64>() ||
        ...
        m_c->setError(NeovimConnector::UnexpectedMsg,
                      tr("Unable to unpack metadata response description, "
    ...
    const int api_compat =
        metadata.value("version").toMap().value("api_compatible").toUInt();
    const int api_level =
        metadata.value("version").toMap().value("api_level").toUInt();
    // qDebug() << "Neovim API version compatible with" << api_compat <<
    // "supported" << api_level;
