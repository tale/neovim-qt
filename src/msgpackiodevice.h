...
class MsgpackIODevice : public QObject {
  ...
  public:
    ...
    typedef QVariant (*msgpackExtDecoder)(MsgpackIODevice *, const char *data,
                                          quint32 size);
  ...
  protected slots:
    void setError(NeovimQt::MsgpackIODevice::MsgpackError err,
                  const QString &msg);
  ...
  private:
    static int msgpack_write_to_stdout(void *data, const char *buf,
                                       unsigned long int len);
    static int msgpack_write_to_dev(void *data, const char *buf,
                                    unsigned long int len);
...
class MsgpackRequestHandler {
  public:
    virtual void handleRequest(MsgpackIODevice *, quint32 msgid,
                               const QByteArray &, const QVariantList &) = 0;
};
