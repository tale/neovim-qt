...
MsgpackIODevice *MsgpackIODevice::fromStdinOut(QObject *parent) {
    ...
    connect(rsn, &StdinReader::dataAvailable, rpc,
            &MsgpackIODevice::dataAvailableStdin);
    rsn->start();
#else
    QSocketNotifier *rsn = new QSocketNotifier(0, QSocketNotifier::Read, rpc);
    connect(rsn, &QSocketNotifier::activated, rpc,
            &MsgpackIODevice::dataAvailableFd);
#endif
    return rpc;
}

MsgpackIODevice::MsgpackIODevice(QIODevice *dev, QObject *parent)
    : QObject(parent), m_reqid(0), m_dev(dev), m_reqHandler(0),
      m_error(NoError) {
    ...
    if (m_dev) {
        ...
        connect(m_dev, &QAbstractSocket::readyRead, this,
                &MsgpackIODevice::dataAvailable);
...
int MsgpackIODevice::msgpack_write_to_stdout(void *data, const char *buf,
...
int MsgpackIODevice::msgpack_write_to_dev(void *data, const char *buf,
                                          unsigned long int len) {
    MsgpackIODevice *c = static_cast<MsgpackIODevice *>(data);
...
void MsgpackIODevice::dataAvailableStdin(const QByteArray &data) {
    if ((quint64)data.length() > msgpack_unpacker_buffer_capacity(&m_uk)) {
        setError(InvalidDevice, tr("Error when reading from stdin, "
                                   "BUG(buffered data exceeds capaciy)"));
        return;
...
void MsgpackIODevice::dispatch(msgpack_object &req) {
    ...
    case 0:
        if (req.via.array.ptr[1].type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
            qDebug() << "Received Invalid request: msg id MUST be a positive "
                        "integer";
    ...
    case 1:
        if (req.via.array.ptr[1].type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
            qDebug() << "Received Invalid response: msg id MUST be a positive "
                        "integer";
            return;
        }
 ...
 */
bool MsgpackIODevice::sendResponse(uint64_t msgid, const QVariant &err,
                                   const QVariant &res) {
    if (!checkVariant(err) || !checkVariant(res)) {
        qDebug() << "Unable to serialize response" << res;
        sendError(msgid,
                  tr("Internal server error, could not serialize response"));
        return false;
    }
...
MsgpackRequest *MsgpackIODevice::startRequestUnchecked(const QString &method,
    ...
    connect(r, &MsgpackRequest::timeout, this,
            &MsgpackIODevice::requestTimeout);
 ...
 */
quint32 MsgpackIODevice::msgId() { return this->m_reqid++; }
...
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in, int64_t &out) {
    if (in.type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
        qWarning() << "Attempting to decode as int64_t when type is" << in.type
                   << in;
        out = -1;
        return true;
    }
...
void MsgpackIODevice::send(double d) { msgpack_pack_float(&m_pk, d); }
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in, double &out) {
    if (in.type != MSGPACK_OBJECT_FLOAT) {
        qWarning() << "Attempting to decode as double when type is" << in.type
                   << in;
        out = -1;
        return true;
    }
...
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in, bool &out) {
    if (in.type != MSGPACK_OBJECT_BOOLEAN) {
        qWarning() << "Attempting to decode as bool when type is" << in.type
                   << in;
        out = false;
        return true;
    }
...
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in,
    ...
    out.clear();
    if (in.type != MSGPACK_OBJECT_ARRAY) {
        qWarning() << "Attempting to decode as QList<double> when type is"
                   << in.type << in;
        return true;
    }
...
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in, QVariant &out) {
    ...
    case MSGPACK_OBJECT_EXT:
        if (m_extTypes.contains(in.via.ext.type)) {
            out = m_extTypes.value(in.via.ext.type)(this, in.via.ext.ptr,
                                                    in.via.ext.size);
            if (!out.isValid()) {
