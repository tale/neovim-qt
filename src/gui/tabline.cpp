...
Tabline::Tabline(NeovimConnector &nvim, QWidget *parent) noexcept
    : m_nvim{nvim}, m_tabline{this}, m_bufferline{this}, m_spacer{this} {
    ...
    connect(&m_nvim, &NeovimConnector::ready, this,
            &Tabline::neovimConnectorReady);
    connect(&m_tabline, &QTabBar::currentChanged, this,
            &Tabline::currentChangedTabline);
    connect(&m_tabline, &QTabBar::tabCloseRequested, this,
            &Tabline::closeRequestedTabline);
    connect(&m_bufferline, &QTabBar::currentChanged, this,
            &Tabline::currentChangedBufline);
    connect(&m_bufferline, &QTabBar::tabCloseRequested, this,
            &Tabline::closeRequestedBufline);

    QSettings settings;
    m_isEnabled =
        settings.value(cs_showTablineOptionName, cs_defaultIsTablineEnabled)
            .toBool();
    m_isBufferlineEnabled =
        settings.value(cs_showBuffersOptionName, true).toBool();
    updateTablineVisibility();
}

void Tabline::neovimConnectorReady() noexcept {
    connect(m_nvim.api0(), &NeovimApi0::neovimNotification, this,
            &Tabline::handleNeovimNotification);
    m_nvim.api0()->vim_subscribe("Gui");
}

void Tabline::handleNeovimNotification(const QByteArray &name,
...
void Tabline::handleRedraw(const QByteArray &name,
                           const QVariantList &args) noexcept {
...
void Tabline::handleGuiOption(const QVariantList &args) noexcept {
    if (args.size() < 2 || !args.at(0).canConvert<QString>() ||
        !args.at(1).canConvert<QString>()) {
        return;
    }
...
static std::vector<Tab> ParseTablineVariant(const QVariantList tabs) noexcept {
    ...
    for (const auto &varTab : tabs) {
        if (static_cast<QMetaType::Type>(varTab.type()) !=
            QMetaType::QVariantMap) {
            qWarning() << "Unexpected varTab value in tabline_update:"
                       << varTab;
            continue;
        }
        ...
        if (!tabMap.contains(key) || !tabMap.contains("name")) {
            qWarning() << "Unexpected tabMap value in tabline_update:"
                       << tabMap;
            continue;
        }
...
void Tabline::handleTablineUpdate(const QVariantList &args) noexcept {
    if (args.size() < 2 || !args.at(0).canConvert<uint64_t>() ||
        static_cast<QMetaType::Type>(args.at(1).type()) !=
    ...
    if (!args.at(2).canConvert<uint64_t>() ||
        static_cast<QMetaType::Type>(args.at(3).type()) !=
            QMetaType::QVariantList) {
        qWarning() << "Unexpected argument for tabline_update:" << args;
        return;
    }
...
void Tabline::handleOptionShowTabline(const QVariantList &args) noexcept {
    ...
    if (args.size() < 2 || !args.at(1).canConvert<int>()) {
        qWarning() << "Tabline unexpected format for option showtabline:"
                   << args;
    }
    ...
    auto OptionFromInteger = [](int value) noexcept -> OptionShowTabline {
        const OptionShowTabline enumValue{
            static_cast<OptionShowTabline>(value)};
        ...
        qWarning() << QStringLiteral(
                          "Error: unrecognized value for showtabline { %1 }")
                          .arg(value);
...
static QIcon GetIconFromFilePath(const QString &path) noexcept {
...

    {
        ...
        auto pathMatchesExact =
            [&](const CachedBufferIcon &cacheEntry) noexcept {
                // We assume paths are case sensitive strings, this is not
                // always correct. False positives may result in duplicate
                // entries, this is okay. Examples: Windows paths, Windows short
                // vs long paths, etc
                return cacheEntry.first == path;
            };

        auto result{std::find_if(s_iconCache.begin(), s_iconCache.end(),
                                 pathMatchesExact)};
        ...
        // Some file-does-not-exist cases display strange icons, use generic
        // text icon
...
static void SetTabIconAndTooltipCallback(QPointer<QTabBar> bufferline,
                                         int bufIndex,
                                         const QVariant &resp) noexcept {
...
void Tabline::drawTablineUpdates(const std::vector<Tab> tabList,
                                 uint64_t curtab,
                                 const std::vector<Tab> &bufferList,
                                 uint64_t curbuf) noexcept {
    updateTabControl(m_tabline, m_nvim.api0(), tabList, curtab,
                     false /*drawTabIcons*/);
    updateTabControl(m_bufferline, m_nvim.api0(), bufferList, curbuf,
                     true /*drawTabIcons*/);
    updateTablineVisibility();
}

void Tabline::updateTabControl(QTabBar &tabControl, NeovimApi0 *nvimApi0,
                               const std::vector<Tab> tabList, uint64_t curtab,
    ...
    for (const auto &tab : tabList) {
        ...
        if (drawTabIcons && nvimApi0) {
            auto reqBufferPath{
                nvimApi0->vim_eval(QStringLiteral("expand('#%1:p')")
                                       .arg(tab.GetHandle())
                                       .toLatin1())};

            QPointer<QTabBar> spTabControl{&tabControl};
            auto handle = [spTabControl, tabIndex](
                              quint32, quint64, const QVariant &resp) noexcept {
...
void Tabline::updateTablineVisibility() noexcept {
    ...
    const bool isLegacyMode{!m_isBufferlineEnabled ||
                            m_bufferline.count() == 0};
...
void Tabline::closeRequestedTabline(int index) noexcept {
    ...
    m_nvim.api0()->vim_command(
        QStringLiteral("tabclose %1").arg(handle).toLatin1());
}
...
void Tabline::currentChangedBufline(int index) noexcept {
    ...
    const uint64_t handle{m_bufferline.tabData(index).toULongLong()};
    m_nvim.api0()->vim_command(
        QStringLiteral("buffer! %1").arg(handle).toLatin1());
}
...
static QString GetSanitizedErrorString(const QVariant &err) noexcept {
    ...
    static const QRegularExpression s_reUnsaved{
        "^.*E89: No write since last change for buffer (\\d+) \\(add \\! to "
        "override\\)$"};
...
void Tabline::handleCloseBufferError(quint32 msgid, quint64 fun,
                                     const QVariant &err) noexcept {
    QMessageBox msgBox;
...
void Tabline::closeRequestedBufline(int index) noexcept {
    ...
    connect(reqCloseBuffer, &MsgpackRequest::error, this,
            &Tabline::handleCloseBufferError);
}
