...
struct ConnectorInitArgs {
    enum class Type { Embed, Server, Spawn, Default };
    ...
    ConnectorInitArgs(const QCommandLineParser &parser,
                      QStringList nvimArgs) noexcept;

    ConnectorInitArgs(Type _type, int _timeout, QString _server, QString _nvim,
                      QStringList nvimArgs) noexcept;
};

ConnectorInitArgs::Type
getConnectorType(const QCommandLineParser &parser) noexcept {
...
ConnectorInitArgs::ConnectorInitArgs(const QCommandLineParser &parser,
                                     QStringList nvimArgs) noexcept
    : type{getConnectorType(parser)}, timeout{parser.value("timeout").toInt()},
      server{parser.value("server")}, nvim{parser.value("nvim")},
      positionalArgs{parser.positionalArguments()}, neovimArgs{
                                                        std::move(nvimArgs)} {}

ConnectorInitArgs::ConnectorInitArgs(Type _type, int _timeout, QString _server,
                                     QString _nvim,
                                     QStringList nvimArgs) noexcept
    : type{_type}, timeout{_timeout}, server{std::move(_server)},
      nvim{std::move(_nvim)}, neovimArgs{std::move(nvimArgs)} {}
...
void onWindowClosing(int status) noexcept { s_exitStatus = status; }

void onWindowDestroyed(QObject *obj) noexcept {
    s_windows.erase(
        std::remove(s_windows.begin(), s_windows.end(), (MainWindow *)obj));
...
MainWindow &createWindow(const ConnectorInitArgs &args) noexcept {
    ...
    QObject::connect(win, &MainWindow::activeChanged, app,
...
void App::processCommandlineOptions(QCommandLineParser &parser,
                                    QStringList arguments) noexcept {
    parser.addOption(QCommandLineOption(
        "nvim", QCoreApplication::translate("main", "nvim executable path"),
    ...
    // Some platforms use --qwindowgeometry, while other platforms use the
    // --geometry. Make the correct help message is displayed.
    if (hasGeometryArg()) {
        parser.addOption(QCommandLineOption(
            "geometry",
    ...
    parser.addOption(QCommandLineOption(
        "fullscreen", QCoreApplication::translate(
                          "main", "Open the window in fullscreen on startup")));
    parser.addOption(QCommandLineOption(
        "embed", QCoreApplication::translate(
                     "main", "Communicate with Neovim over stdin/out")));
    parser.addOption(
        QCommandLineOption("server",
                           QCoreApplication::translate(
                               "main", "Connect to existing Neovim instance"),
                           QCoreApplication::translate("main", "addr")));
    parser.addOption(QCommandLineOption(
        "spawn", QCoreApplication::translate(
                     "main", "Treat positional arguments as the nvim argv")));
    parser.addOption(QCommandLineOption(
        {"v", "version"},
...
#ifdef Q_OS_UNIX
    parser.addOption(QCommandLineOption(
        "nofork", QCoreApplication::translate("main", "Run in foreground")));
#endif

    parser.addPositionalArgument(
        "file", QCoreApplication::translate("main", "Edit specified file(s)"),
        "[file...]");
    parser.addPositionalArgument(
        "-- [nvim_args]",
...
void App::checkArgumentsMayTerminate(QCommandLineParser &parser) noexcept {
    ...
    int exclusive =
        parser.isSet("server") + parser.isSet("embed") + parser.isSet("spawn");
    if (exclusive > 1) {
        qWarning()
            << "Options --server, --spawn and --embed are mutually exclusive\n";
        ::exit(-1);
    }
    ...
    if (!parser.positionalArguments().isEmpty() &&
        ...
        qWarning()
            << "--embed and --server do not accept positional arguments\n";
        ::exit(-1);
    }
    ...
    if (!valid_timeout || timeout_opt <= 0) {
        qWarning() << "Invalid argument for --timeout"
...
/*static*/ QString App::getRuntimePath() noexcept {
    ...
    const QDir d{
        QDir{applicationDirPath()}.filePath(NVIM_QT_RELATIVE_RUNTIME_PATH)};
    if (d.exists()) {
        return d.path();
    }
...
/*static*/ QStringList App::getNeovimArgs() noexcept {
    ...
    return {"--cmd", QString{"let &rtp.=',%1'"}.arg(runtimePath), "--cmd",
            "set termguicolors"};
}
...
void App::showVersionInfo(QCommandLineParser &parser) noexcept {
    ...
    const QString nvimExecutable{(parser.isSet("nvim")) ? parser.value("nvim")
                                                        : "nvim"};
...
void App::openNewWindow(const QVariantList &args) noexcept {
    ...
    ConnectorInitArgs params{type, 2000, std::move(server), std::move(nvim),
                             getNeovimArgs()};
