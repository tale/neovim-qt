...
void ScrollBar::neovimConnectorReady() noexcept {
    connect(m_nvim->api0(), &NeovimApi0::neovimNotification, this,
            &ScrollBar::handleNeovimNotification);
...
void ScrollBar::handleNeovimNotification(const QByteArray &name,
...
void ScrollBar::handleRedraw(const QByteArray &name,
                             const QVariantList &args) noexcept {
    if (name == "grid_scroll") {
...
void ScrollBar::setIsVisible(bool isVisible) { setVisible(isVisible); }

void ScrollBar::setAbsolutePosition(uint64_t minLine, uint64_t bufferSize,
...
void ScrollBar::handleWinViewport(const QVariantList &args) noexcept {
    if (args.size() < 6 || !args.at(0).canConvert<uint64_t>() ||
    ...
    setAbsolutePosition(topline, m_lastKnownBufferSize,
                        m_lastKnownWindowHeight);
}

void ScrollBar::handleCursorMoved(const QVariantList &args) noexcept {
    if (args.size() < 4 || !args.at(1).canConvert<uint64_t>() ||
        !args.at(2).canConvert<uint64_t>() ||
        !args.at(3).canConvert<uint64_t>()) {
...
void ScrollBar::handleSetScrollBarVisible(const QVariantList &args) noexcept {
    if (args.size() < 2 || !args.at(1).canConvert<bool>()) {
