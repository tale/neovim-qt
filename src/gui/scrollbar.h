...
class ScrollBar : public QScrollBar {
  ...
  public:
    ...
    void handleRedraw(const QByteArray &name,
                      const QVariantList &opargs) noexcept;

  public slots:
    void setIsVisible(bool isVisible);
    void setAbsolutePosition(uint64_t minLine, uint64_t bufferSize,
  ...
  private:
    ...
    /// Locks out GUI triggered scrolling for a number of lines. Prevent double
    /// registration.
    int m_lineScrollLockout{0};

    /// Last known cursor position. Cursor position is in absolute units, and
    /// scroll commands are sent with relative units. Necessary for conversion.
    int m_lastKnownPosition{0};

    /// Workaround for win_viewport. Saves buffer size from last cursor
    /// movement.
    int m_lastKnownBufferSize{0};

    /// Workaround for win_viewport. Saves window height from last cursor
    /// movement.
