...
static QString DefaultWindowTitle() noexcept { return "Neovim"; }

MainWindow::MainWindow(NeovimConnector *c, QWidget *parent) noexcept
    : QMainWindow{parent}, m_tabline{*c, this}, m_defaultFont{font()},
      m_defaultPalette{palette()} {
    m_errorWidget = new ErrorWidget();
    m_stack.addWidget(m_errorWidget);
    connect(m_errorWidget, &ErrorWidget::reconnectNeovim, this,
            &MainWindow::reconnectNeovim);
...
void MainWindow::init(NeovimConnector *c) {
    ...
    // QSplitter does not allow layouts directly: QWidget { HLayout {
    // ShellWidget, QScrollBar } }
    ...
    connect(m_shell, &Shell::neovimAttachmentChanged, this,
            &MainWindow::handleNeovimAttachment);
    connect(m_shell, SIGNAL(neovimTitleChanged(QString)), this,
            SLOT(neovimSetTitle(QString)));
    connect(m_shell, &Shell::neovimResized, this,
            &MainWindow::neovimWidgetResized);
    connect(m_shell, &Shell::neovimMaximized, this,
            &MainWindow::neovimMaximized);
    connect(m_shell, &Shell::neovimForeground, this,
            &MainWindow::neovimForeground);
    connect(m_shell, &Shell::neovimSuspend, this, &MainWindow::neovimSuspend);
    connect(m_shell, &Shell::neovimFullScreen, this,
            &MainWindow::neovimFullScreen);
    connect(m_shell, &Shell::neovimFrameless, this,
            &MainWindow::neovimFrameless);
    connect(m_shell, &Shell::neovimGuiCloseRequest, this,
            &MainWindow::neovimGuiCloseRequest);
    connect(m_shell, &Shell::neovimOpacity, this,
            &MainWindow::setWindowOpacity);
    connect(m_nvim, &NeovimConnector::processExited, this,
            &MainWindow::neovimExited);
    connect(m_nvim, &NeovimConnector::error, this, &MainWindow::neovimError);
    connect(m_shell, &Shell::neovimIsUnsupported, this,
            &MainWindow::neovimIsUnsupported);
    connect(m_shell, &Shell::neovimShowContextMenu, m_contextMenu,
            &ContextMenu::showContextMenu);

    // GuiAdaptive Color/Font/Style Signal/Slot Connections
    connect(m_shell, &Shell::setGuiAdaptiveColorEnabled, this,
            &MainWindow::setGuiAdaptiveColorEnabled);
    connect(m_shell, &Shell::setGuiAdaptiveFontEnabled, this,
            &MainWindow::setGuiAdaptiveFontEnabled);
    connect(m_shell, &Shell::setGuiAdaptiveStyle, this,
            &MainWindow::setGuiAdaptiveStyle);
    connect(m_shell, &Shell::showGuiAdaptiveStyleList, this,
            &MainWindow::showGuiAdaptiveStyleList);
    connect(m_shell, &Shell::colorsChanged, this,
            &MainWindow::updateAdaptiveColor);
...
void MainWindow::neovimExited(int status) {
    if (m_nvim->errorCause() != NeovimConnector::NoError) {
        ...
        m_errorWidget->setText(
            QString("Neovim exited with status code (%1)").arg(status));
        m_errorWidget->showReconnect(m_nvim->canReconnect());
        m_stack.setCurrentIndex(0);
    } else {
        close();
...
void MainWindow::neovimError(NeovimConnector::NeovimError err) {
    ...
    case NeovimConnector::FailedToStart:
        m_errorWidget->setText(
            "Unable to start nvim: " + m_nvim->errorString() + "<br>(" +
            m_nvim->connectionDescription() + ")");
        break;
    default:
        m_errorWidget->setText(m_nvim->errorString() + "<br>(" +
                               m_nvim->connectionDescription() + ")");
    }
    m_errorWidget->showReconnect(m_nvim->canReconnect());
    m_stack.setCurrentIndex(0);
...
void MainWindow::neovimIsUnsupported() {
    m_errorWidget->setText(QString("Cannot connect to this Neovim, required "
                                   "API version 1, found [%1-%2]")
...
Shell *MainWindow::shell() { return m_shell; }
...
void MainWindow::setGuiAdaptiveStyle(const QString &styleName) {
    ...
    auto childrenWidgets{findChildren<QWidget *>() +
                         m_shell->findChildren<QWidget *>()};
...
void MainWindow::updateAdaptiveFont() noexcept {
    ...
    const QFont &font{(m_isAdaptiveFontEnabled) ? m_shell->font()
                                                : m_defaultFont};
...
void MainWindow::updateAdaptiveColor() noexcept {
    ...
    if (isPopupMenuSupported) {
        ...
        const HighlightAttribute pmenusel{
            m_shell->GetHighlightGroup("PmenuSel")};
        palette.setColor(QPalette::Highlight, pmenusel.GetBackgroundColor());
        palette.setColor(QPalette::HighlightedText,
                         pmenusel.GetForegroundColor());
