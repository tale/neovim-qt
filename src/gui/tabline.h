...
class Tabline : public QToolBar {
  ...
  private:
    ...
    void handleCloseBufferError(quint32 msgid, quint64 fun,
                                const QVariant &err) noexcept;

    void drawTablineUpdates(const std::vector<Tab> tabList, uint64_t curtab,
                            const std::vector<Tab> &bufferList,
                            uint64_t curbuf) noexcept;

    void updateTabControl(QTabBar &tabControl, NeovimApi0 *nvimApi0,
