...
/// needed.
void ShellContents::allocData() { _data = new Cell[_rows * _columns]; }
...
/// the scrolled area.
void ShellContents::scrollRegion(int row0, int row1, int col0, int col1,
                                 int count) {
    if (count == 0) {
        return;
    }
    if (!verifyRegion(row0, row1, col0, col1)) {
        qDebug() << "Scroll region is invalid (row0, row1, col0, col1)" << row0
                 << row1 << col0 << col1;
        return;
    }
...
void ShellContents::resize(int newRows, int newColumns) {
    ...
    for (int i = 0; i < copyRows; i++) {
        memcpy(&_data[i * _columns], &old[i * oldColumns],
...
const Cell *ShellContents::data() { return _data; }
