...
void Test::fontDescriptionFromQFont() {
    ...
    QCOMPARE(ShellWidget::fontDesc(
                 {fontFamily, 16, arbitraryWeight, true /*italic*/}),
             QString{"%1:h16:w%2:i"}.arg(fontFamily).arg(arbitraryWeight));
}

static bool ValidateQFont(const QVariant &varFont, const QString &fontFamily,
                          qreal pointSizeF, int weight, bool italic) noexcept {
    ...
    if (font.family() != fontFamily || font.pointSizeF() != pointSizeF ||
        font.weight() != weight || font.italic() != italic) {
        return false;
    }
...
void Test::fontDescriptionToQFont() {
    ...
    //
    QVERIFY(ValidateQFont(ShellWidget::TryGetQFontFromDescription(
                              QString{"%1:h11"}.arg(fontFamily), defaultFont),
                          fontFamily, 11, QFont::Weight::Normal,
                          false /*italic*/));

    QVERIFY(ValidateQFont(ShellWidget::TryGetQFontFromDescription(
                              QString{"%1:h12:l"}.arg(fontFamily), defaultFont),
                          fontFamily, 12, QFont::Weight::Light,
                          false /*italic*/));

    QVERIFY(ValidateQFont(
        ShellWidget::TryGetQFontFromDescription(
            QString{"%1:h14:sb"}.arg(fontFamily), defaultFont),
        fontFamily, 14, QFont::Weight::DemiBold, false /*italic*/));

    QVERIFY(ValidateQFont(ShellWidget::TryGetQFontFromDescription(
                              QString{"%1:h16:b"}.arg(fontFamily), defaultFont),
                          fontFamily, 16, QFont::Weight::Bold,
                          false /*italic*/));

    const int arbitraryWeight{QFont::Weight::Normal + 1};
    QVERIFY(ValidateQFont(
        ShellWidget::TryGetQFontFromDescription(
            QString{"%1:h16:w%2"}.arg(fontFamily).arg(arbitraryWeight),
            defaultFont),
    ...
    const QVariant varInvalidLargeWeight{
        ShellWidget::TryGetQFontFromDescription(
            QString{"%1:h16:w%2"}.arg(fontFamily).arg(10000), defaultFont)};
    ...
    const QVariant varInvalidNegativeWeight{
        ShellWidget::TryGetQFontFromDescription(
            QString{"%1:h16:w%2"}.arg(fontFamily).arg(-1), defaultFont)};

    QVERIFY(!ShellWidget::IsValidFont(varInvalidNegativeWeight));
    QCOMPARE(varInvalidNegativeWeight,
             QVariant{QString{"Invalid font weight"}});

    const QVariant varInvalidHeight{ShellWidget::TryGetQFontFromDescription(
        QString{"FooBarInvalidFont:ha"}, defaultFont)};
    ...
    const QVariant varInvalidNegativeHeight{
        ShellWidget::TryGetQFontFromDescription(
            QString{"FooBarInvalidFont:h-1"}, defaultFont)};
