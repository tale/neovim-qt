...
ShellWidget::ShellWidget(QWidget *parent) : QWidget(parent) {
    ...
    connect(&m_cursor, &Cursor::CursorChanged, this,
            &ShellWidget::handleCursorChanged);
}
...
void ShellWidget::setDefaultFont() {
    static const QFont font{getDefaultFontFamily(), 11 /*pointSize*/,
                            -1 /*weight*/, false /*italic*/};
    setFont(font);
    setCellSize();
}
...
bool ShellWidget::setShellFont(const QFont &font, bool force) noexcept {
    ...
    // This case has always been hit, but results in user visible error messages
    // for recent releases. It is safe to ignore this case, which occurs at
    // startup time.
    ...
    if (!force) {
        if (!fi.fixedPitch()) {
            emit fontError(QStringLiteral("%1 is not a fixed pitch font")
                               .arg(font.family()));
            return false;
        }
        ...
        if (isBadMonospace(font)) {
            emit fontError(
                QString("Warning: Font \"%1\" reports bad fixed pitch metrics")
                    .arg(font.family()));
        }
    }
...
void ShellWidget::setFont(const QFont &f) { QWidget::setFont(f); }
...
QSize ShellWidget::cellSize() const { return m_cellSize; }
...
QRect ShellWidget::getNeovimCursorRect(QRect cellRect) noexcept {
    ...
    case Cursor::Shape::Vertical: {
        ...
        cursorRect.setWidth(m_cellSize.width() * m_cursor.GetPercentage() /
                            100);
    } break;
    }
...
void ShellWidget::paintNeovimCursorBackground(QPainter &p,
    ...
    // If the window does not have focus, draw an outline around the cursor
    // cell.
    if (!hasFocus()) {
...
void ShellWidget::paintNeovimCursorForeground(
    QPainter &p, QRect cellRect, QPoint pos,
...
void ShellWidget::paintUnderline(QPainter &p, const Cell &cell,
                                 QRect cellRect) noexcept {
    if (!cell.IsUnderline()) {
        return;
    }
...
static QPainterPath GetUndercurlPath(QRect cellRect, qreal singleCellWidth,
                                     qreal penWidth) noexcept {
    // To draw the undercurl, we use a sine curve that is defined in terms of
    // the screen coordinates. This prevents discontinuities at cell boundaries.

    // The parameters are chosen so that the curve crosses the cell boundary at
    // a 45 degree angle. This looks visually pleasing. The curve is scaled so
    // that 3/2 periods fit in a single cell (experimentally chosen to obtain a
    // curl that is neither to large nor too small). The minimum function
    // ensures that the curl doesn't oscillate too much at small font sizes and
    // remains visible.
    ...
    QPainterPath path{
        QPointF(cellRect.left(), y0 + amp * qSin(cellRect.left() * scale))};
...
void ShellWidget::paintUndercurl(QPainter &p, const Cell &cell,
                                 QRect cellRect) noexcept {
    if (!cell.IsUndercurl()) {
        return;
    }
    ...
    p.drawPath(
        GetUndercurlPath(cellRect, m_cellSize.width(), specialPen.widthF()));
...
void ShellWidget::paintStrikeThrough(QPainter &p, const Cell &cell,
                                     QRect cellRect) noexcept {
...
void ShellWidget::paintBackgroundClearCell(QPainter &p, const Cell &cell,
                                           QRect cellRect,
...
QFont ShellWidget::GetCellFont(const Cell &cell) const noexcept {
    ...
    if (cell.IsDoubleWidth()) {
        ...
        auto wideFont =
            std::find_if(m_guifontwidelist.begin(), m_guifontwidelist.end(),
                         isCharacterInWideFont);
    ...
    cellFont.setStyleHint(QFont::TypeWriter,
...
void ShellWidget::paintForegroundCellText(QPainter &p, const Cell &cell,
                                          QRect cellRect,
                                          bool isCursorCell) noexcept {
...
static bool AreGlyphPositionsUniform(const QVector<QPointF> &glyphPositionList,
...
DistributeGlyphPositions(QVector<QPointF> &&glyphPositionList,
                         int cellWidth) noexcept {
    if (glyphPositionList.size() > 1) {
...
static QVector<uint32_t>
RemoveLigaturesUnderCursor(const QGlyphRun &glyphRun,
                           const QString &textGlyphRun,
                           int cursorGlyphRunPos) noexcept {
    auto glyphIndexList{glyphRun.glyphIndexes()};
    auto glyphIndexListNoLigatures{
        glyphRun.rawFont().glyphIndexesForString(textGlyphRun)};

    if (cursorGlyphRunPos < 0 || cursorGlyphRunPos >= glyphIndexList.size() ||
        cursorGlyphRunPos >= glyphIndexListNoLigatures.size()) {
        qDebug() << "ERROR: Invalid cursorGlyphRunPos!";
        return {};
    }

    if (glyphIndexList.at(cursorGlyphRunPos) !=
        glyphIndexListNoLigatures.at(cursorGlyphRunPos)) {
...
void ShellWidget::paintForegroundTextBlock(QPainter &p, const Cell &cell,
                                           QRect blockRect, const QString &text,
    ...
    for (auto &glyphRun : textLayout.glyphRuns()) {
        ...
        const int cellWidth{(cell.IsDoubleWidth()) ? m_cellSize.width() * 2
                                                   : m_cellSize.width()};
        ...
        if (!AreGlyphPositionsUniform(glyphPositionList, cellWidth)) {
            glyphRun.setPositions(DistributeGlyphPositions(
                std::move(glyphPositionList), cellWidth));
        }

        const bool isCursorVisibleInGlyphRun{
            m_cursor.IsVisible() && cursorPos >= 0 &&
        ...
        // When the cursor IS within the glyph run, decompose individual
        // characters under the cursor.
        const int cursorGlyphRunPos{cursorPos - glyphsRendered};
        const QString textGlyphRun{
            QStringRef{&text, glyphsRendered, sizeGlyphRun}.toString()};

        // Compares a glyph run with and without ligatures. Ligature glyphs are
        // detected as differences in these two lists. A non-empty
        // newCursorGlyphList indicates glyph substitution is required.
        const auto newCursorGlyphList{RemoveLigaturesUnderCursor(
        ...
        paintNeovimCursorForeground(
            p, cursorCellRect,
            glyphRun.positions().at(cursorGlyphRunPos).toPoint() + pos,
...
void ShellWidget::paintEvent(QPaintEvent *ev) {
    ...
    for (auto rect{ev->region().cbegin()}; rect != ev->region().cend();
         rect++) {
    ...
    for (auto margin{marginsIntersected.cbegin()};
         margin != marginsIntersected.cend(); margin++) {
    ...
    for (int i = start_row; i <= end_row; i++) {
        for (int j = end_col; j >= start_col; j--) {
            ...
            if (j <= 0 || !contents().constValue(i, j - 1).IsDoubleWidth()) {
                ...
                const bool isCursorVisibleAtCell{m_cursor.IsVisible() &&
                                                 m_cursor_pos == curPos};
...
void ShellWidget::paintRectLigatures(QPainter &p, const QRect rect) noexcept {
    ...
    for (int i = start_row; i <= end_row; i++) {
        for (int j = start_col; j <= end_col; j++) {
            ...
            QRect blockRect{firstCellRect.topLeft(),
                            lastCellRect.bottomRight()};

            paintBackgroundClearCell(p, firstCell, blockRect, false);
            paintForegroundTextBlock(p, firstCell, blockRect, blockText,
                                     blockCursorPos);
...
void ShellWidget::setSpecial(const QColor &color) { m_spColor = color; }

QColor ShellWidget::special() const { return m_spColor; }

void ShellWidget::setBackground(const QColor &color) { m_bgColor = color; }
...
void ShellWidget::setForeground(const QColor &color) { m_fgColor = color; }
...
const ShellContents &ShellWidget::contents() const { return m_contents; }

/// Put text in position, returns the amount of colums used
int ShellWidget::put(const QString &text, int row, int column, QColor fg,
                     QColor bg, QColor sp, bool bold, bool italic,
                     bool underline, bool undercurl, bool strikethrough,
                     bool reverse) {
    HighlightAttribute hl_attr = {
        fg, bg, sp, reverse, italic, bold, underline, undercurl, strikethrough};
...
void ShellWidget::scrollShellRegion(int row0, int row1, int col0, int col1,
                                    int rows) {
    if (rows != 0) {
...
QRect ShellWidget::absoluteShellRect(int row0, int col0, int rowcount,
...
int ShellWidget::rows() const { return m_contents.rows(); }

int ShellWidget::columns() const { return m_contents.columns(); }
...
void ShellWidget::setNeovimCursor(uint64_t row, uint64_t col) noexcept {
    ...
    update((isLigatureModeEnabled()) ? absoluteShellRectRow(row)
                                     : neovimCursorRect());
}
...
void ShellWidget::handleCursorChanged() { update(neovimCursorRect()); }

QString ShellWidget::fontDesc() const noexcept { return fontDesc(font()); }

/*static*/ QString ShellWidget::fontDesc(const QFont &font) noexcept {
    QString fdesc{
        QStringLiteral("%1:h%2").arg(font.family()).arg(font.pointSize())};
...
QVariant
ShellWidget::TryGetQFontFromDescription(const QString &fdesc) const noexcept {
    return TryGetQFontFromDescription(fdesc, font());
}

/*static*/ QVariant
ShellWidget::TryGetQFontFromDescription(const QString &fdesc,
                                        const QFont &curFont) noexcept {
...
/*static*/ bool ShellWidget::isBadMonospace(const QFont &f) noexcept {
    ...
    if (fm_normal.averageCharWidth() != fm_normal.maxWidth()) {
        QFontInfo info(f);
        qDebug() << f.family()
                 << "Average and Maximum font width mismatch for Regular font; "
                    "QFont::exactMatch() is"
                 << f.exactMatch() << "Real font is " << info.family()
                 << info.pointSize();
        return true;
    }
    ...
    if (fm_italic.averageCharWidth() != fm_italic.maxWidth() ||
        ...
        QFontInfo info(fi);
        qDebug() << fi.family()
                 << "Average and Maximum font width mismatch for Italic font; "
                    "QFont::exactMatch() is"
                 << fi.exactMatch() << "Real font is " << info.family()
                 << info.pointSize();
        return true;
    }
    ...
    if (fm_bold.averageCharWidth() != fm_bold.maxWidth() ||
        ...
        QFontInfo info(fb);
        qDebug() << fb.family()
                 << "Average and Maximum font width mismatch for Bold font; "
                    "QFont::exactMatch() is"
                 << fb.exactMatch() << "Real font is " << info.family()
                 << info.pointSize();
        return true;
    }
    ...
    if (fm_boldit.averageCharWidth() != fm_boldit.maxWidth() ||
        ...
        QFontInfo info(fbi);
        qDebug() << fbi.family()
                 << "Average and Maximum font width mismatch for Bold+Italic "
                    "font; QFont::exactMatch() is"
                 << fbi.exactMatch() << "Real font is " << info.family()
                 << info.pointSize();
        return true;
    }
