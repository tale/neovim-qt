...
class ShellWidget : public QWidget {
    ...
    Q_PROPERTY(bool ligatureMode MEMBER m_isLigatureModeEnabled READ
                   isLigatureModeEnabled WRITE setLigatureMode NOTIFY
                       ligatureModeChanged)

  public:
    ShellWidget(QWidget *parent = 0);

    /// Represents Neovim 'background' option. Used for rendering default theme
    /// colors (-1). See 'default_colors_set' in Neovim ':help ui-linegrid' for
    /// more details.
    enum class Background : int { Dark, Light };
    ...
    static QVariant TryGetQFontFromDescription(const QString &fdesc,
    ...
    ///
    /// This function is not perfect and some broken fonts may still return
    /// false, or font substitution may cause good fonts to fail. The font
    /// max/average metrics are compared with the italic/bold double width
    /// variants.
    static bool isBadMonospace(const QFont &f) noexcept;

    /// Font ligatures do not render by default, since each character is an
    /// individual entity on the Neovim grid. When ligatureMode is enabled,
    /// segments of text on a given line with the same style are rendered
    /// together. This allows the display of font ligatures such as those found
    /// in FiraCode.
  ...
  public slots:
    ...
    int put(const QString &text, int row, int column, QColor fg = {},
            QColor bg = {}, QColor sp = {}, bool bold = false,
            bool italic = false, bool underline = false, bool undercurl = false,
            bool strikethrough = false, bool reverse = false);

    int put(const QString &text, int row, int column,
  ...
  protected:
    ...
    QRect absoluteShellRect(int row0, int col0, int rowcount,
  ...
  private:
    ...
    void paintNeovimCursorForeground(QPainter &p, QRect cellRect, QPoint pos,
    ...
    void paintStrikeThrough(QPainter &p, const Cell &cell,
                            QRect cellRect) noexcept;
    void paintBackgroundClearCell(QPainter &p, const Cell &cell, QRect cellRect,
                                  bool isCursorCell) noexcept;
    void paintForegroundCellText(QPainter &p, const Cell &cell, QRect cellRect,
                                 bool isCursorCell) noexcept;

    void paintForegroundTextBlock(QPainter &p, const Cell &cell,
