...
static QVariant GetButtonName(Qt::MouseButton bt, uint8_t clickCount) noexcept {
...
QString convertMouse(Qt::MouseButton bt, QEvent::Type type,
    ...
    return QStringLiteral("<%1%2%3><%4,%5>")
        .arg(GetModifierPrefix(mod), varButtonName.toString(),
             GetEventString(type), xPos, yPos);
}

static QString ToKeyString(const QString &modPrefix,
...
QString convertKey(const QKeyEvent &ev) noexcept {
    ...
    if (text == "<") {
        const Qt::KeyboardModifiers modNoShift{
    ...
    if (text.isEmpty()) {
        ...
        if (key == Qt::Key::Key_Alt || key == Qt::Key::Key_AltGr ||
            key == Qt::Key::Key_CapsLock || key == Qt::Key::Key_Control ||
            key == Qt::Key::Key_Meta || key == Qt::Key::Key_Shift ||
            key == Qt::Key::Key_Super_L || key == Qt::Key::Key_Super_R ||
        ...
        if (key == Qt::Key::Key_VolumeDown || key == Qt::Key::Key_VolumeMute ||
    ...
    if ((c.unicode() >= 0x80 || c.isPrint()) && !(mod & ControlModifier()) &&
        !(mod & CmdModifier())) {
    ...
    QKeyEvent evNormalized{
        CreatePlatformNormalizedKeyEvent(ev.type(), key, mod, text)};
