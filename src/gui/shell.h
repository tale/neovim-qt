...
class Shell : public ShellWidget {
    Q_OBJECT
    Q_PROPERTY(bool isNeovimBusy READ isNeovimBusy()
                   NOTIFY neovimBusyChanged(bool))
    Q_PROPERTY(bool isNeovimAttached READ isNeovimAttached()
                   NOTIFY neovimAttachmentChanged(bool))
  public:
    Shell(NeovimConnector *nvim, QWidget *parent = 0);
    ~Shell();
    QSize sizeIncrement() const;
    virtual QVariant
        inputMethodQuery(Qt::InputMethodQuery) const Q_DECL_OVERRIDE;

    bool isNeovimBusy() const noexcept { return m_neovimBusy; }

    bool isNeovimAttached() const noexcept { return m_attached; }

    PopupMenu &getPopupMenu() noexcept { return m_pum; }

    /// Convert QWheelEvent to Neovim wheel event string. Testable.
    /// NOTE: Sums and stores partial wheel events in outparam
    /// `scrollRemainderOut`
    ...
    template <class T>
    static void DispatchRedrawNotifications(T *pThis,
  ...
  signals:
    ...
    /// This signal is emmited if the running neovim version is unsupported by
    /// the GUI
  ...
  protected slots:
    ...
    void handleGetBackgroundOption(quint32 msgid, quint64 fun,
                                   const QVariant &val);
  ...
  protected:
    ...
    void handleGuiFontOption(quint32 msgid, quint64 fun,
                             const QVariant &val) noexcept;
    void handleGuiFontVariable(quint32 msgid, quint64 fun,
                               const QVariant &val) noexcept;
...
class ShellRequestHandler : public QObject, public MsgpackRequestHandler {
  ...
  public:
    ...
    virtual void handleRequest(MsgpackIODevice *dev, quint32 msgid,
...
template <class T>
/*static*/ void
Shell::DispatchRedrawNotifications(T *pThis,
                                   const QVariantList &args) noexcept {
    for (const auto &update_item : args) {
        ...
        for (const auto &opargs_var : update_args) {
            if (!opargs_var.canConvert<QVariantList>()) {
                qWarning()
                    << "Received unexpected redraw arguments, expecting list"
                    << opargs_var;
                continue;
            }
