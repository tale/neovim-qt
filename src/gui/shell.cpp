...
static ShellOptions GetShellOptionsFromQSettings() noexcept {
    ...
    QVariant ext_linegrid{
        settings.value("ext_linegrid", opts.IsLineGridEnabled())};
    QVariant ext_popupmenu{
        settings.value("ext_popupmenu", opts.IsPopupmenuEnabled())};
    QVariant ext_tabline{
        settings.value("ext_tabline", opts.IsTablineEnabled())};
...
Shell::Shell(NeovimConnector *nvim, QWidget *parent)
    : ShellWidget{parent}, m_nvim{nvim}, m_options{
                                             GetShellOptionsFromQSettings()} {
    ...
    connect(&m_mouseclick_timer, &QTimer::timeout, this,
            &Shell::mouseClickReset);
    ...
    connect(m_nvim, &NeovimConnector::error, this, &Shell::neovimError);
    connect(m_nvim, &NeovimConnector::processExited, this,
            &Shell::neovimExited);
...
bool Shell::setGuiFont(const QString &fdesc, bool force) noexcept {
    ...
    if (isGuiDialogRequest) {
        ...
        QFont font{QFontDialog::getFont(&fontDialogSuccess, QWidget::font(),
                                        this, {},
...
void Shell::updateGuiFontRegisters() noexcept {
    ...
    connect(getOption, &MsgpackRequest::finished, this,
            &Shell::handleGuiFontOption);

    // Update `:GuiFont`, but only if value changes
    MsgpackRequest *getVariable{m_nvim->api0()->vim_get_var("GuiFont")};
    connect(getVariable, &MsgpackRequest::finished, this,
            &Shell::handleGuiFontVariable);
}
...
void Shell::handleGuiFontOption(quint32 msgid, quint64 fun,
...
void Shell::handleGuiFontVariable(quint32 msgid, quint64 fun,
                                  const QVariant &val) noexcept {
    const QString oldFont{val.toString()};
    const QString newFont{fontDesc()};
...
void Shell::setAttached(bool attached) {
    ...
    if (attached) {
        ...
        MsgpackRequest *req_shim{
            m_nvim->api0()->vim_command("runtime plugin/nvim_gui_shim.vim")};
        connect(req_shim, &MsgpackRequest::error, this,
                &Shell::handleShimError);

        MsgpackRequest *req_ginit{
            m_nvim->api0()->vim_command(GetGVimInitCommand())};
        connect(req_ginit, &MsgpackRequest::error, this,
                &Shell::handleGinitError);

        // Neovim was not able to open urls till now. Check if we have any to
        // open.
        if (!m_deferredOpen.isEmpty()) {
            openFiles(m_deferredOpen);
            m_deferredOpen.clear(); // Neovim may change state. Clear to prevent
                                    // reopening.
        }
    }
    ...
    // Issue #868: When GuiFont is loaded on startup, m_nvim is not attached
    // yet. We are connected now, loop-back and update the registers with the
    // correct values.
    updateGuiFontRegisters();
...
void Shell::init() {
    ...
    connect(m_nvim->api0(), &NeovimApi0::on_ui_try_resize, this,
            &Shell::neovimResizeFinished);
    ...
    connect(req, &MsgpackRequest::timeout, m_nvim,
            &NeovimConnector::fatalTimeout);
    ...
    // Set initial value
    m_nvim->api0()->vim_set_var("GuiWindowFrameless",
                                (windowFlags() & Qt::FramelessWindowHint) ? 1
                                                                          : 0);
}
...
void Shell::handlePut(const QVariantList &args) {
    ...
    if (text.isEmpty() && m_cursor_pos.x() > 0 &&
        contents()
            .constValue(m_cursor_pos.y(), m_cursor_pos.x() - 1)
            .IsDoubleWidth()) {
    ...
    if (!text.isEmpty()) {
        int cols =
            put(text, m_cursor_pos.y(), m_cursor_pos.x(), m_hg_foreground,
                m_hg_background, m_hg_special, m_font_bold, m_font_italic,
...
void Shell::handleRedraw(const QByteArray &name, const QVariantList &opargs) {
    if (name == "update_fg") {
        ...
        clearRegion(m_cursor_pos.y(), m_cursor_pos.x(), m_cursor_pos.y() + 1,
                    columns());
...
void Shell::handlePopupMenuShow(const QVariantList &opargs) {
    ...
    if (opargs.size() < 4 ||
        static_cast<QMetaType::Type>(opargs.at(0).type()) !=
        ...
        qWarning() << "Unexpected 5th argument for popupmenu_show:"
                   << opargs.at(4);
        return;
    }
    ...
    // const int64_t grid = (opargs.size() < 5) ? 0 :
    // opargs.at(4).toULongLong();

    QList<PopupMenuItem> model;
    for (const auto &v : items) {
        QVariantList item = v.toList();
        // Item is (text, kind, extra, info)
        if (item.size() < 4 || item.isEmpty() ||
...
void Shell::handlePopupMenuSelect(const QVariantList &opargs) {
    if (opargs.size() < 1 || !opargs.at(0).canConvert<int64_t>()) {
...
void Shell::handleModeChange(const QVariantList &opargs) {
    ...
    if (!cursor_shape.isNull() && cursor_shape.canConvert<QByteArray>()) {
        const QString cursor_shape{
            modePropertyMap.value("cursor_shape").toByteArray()};
    ...
    if (!cell_percentage.isNull() && cell_percentage.canConvert<uint32_t>()) {
        const uint32_t cell_percentage{
            modePropertyMap.value("cell_percentage").toUInt()};
...
void Shell::handleModeInfoSet(const QVariantList &opargs) {
    if (opargs.size() < 2 || !opargs.at(0).canConvert<bool>() ||
        static_cast<QMetaType::Type>(opargs.at(1).type()) !=
            QMetaType::QVariantList) {
    ...
    if (name == "Gui" && args.size() > 0) {
        ...
        if (guiEvName == "Font") {
            ...
            if (reg_name != "*" && reg_name != "+") {
                m_nvim->api0()->vim_report_error(
                    m_nvim->encode("Cannot set register via GUI"));
                return;
            }
            ...
            qDebug() << "Neovim changed clipboard" << data << type << reg_name
                     << clipboard;
...
void Shell::handleMouse(bool enabled) { m_mouseEnabled = enabled; }

void Shell::handleGuiFontFunction(const QVariantList &args) {
    if (args.size() < 2 || !args.at(1).canConvert<QByteArray>()) {
        return;
    }
...
void Shell::handleGridResize(const QVariantList &opargs) {
    if (opargs.size() < 3 || !opargs.at(0).canConvert<qint64>() ||
...
void Shell::handleDefaultColorsSet(const QVariantList &opargs) {
    if (opargs.size() < 5 || !opargs.at(0).canConvert<qint64>() ||
        !opargs.at(1).canConvert<qint64>() ||
        !opargs.at(2).canConvert<qint64>() ||
    ...
    connect(getBackgroundMode, &MsgpackRequest::finished, this,
            &Shell::handleGetBackgroundOption);
...
void Shell::handleHighlightAttributeDefine(const QVariantList &opargs) {
    if (opargs.size() < 4 || !opargs.at(0).canConvert<qint64>() ||
        ...
        static_cast<QMetaType::Type>(opargs.at(2).type()) !=
            QMetaType::QVariantMap) {
...
void Shell::handleHighlightGroupSet(const QVariantList &opargs) noexcept {
    if (opargs.size() < 2 || opargs.at(0).type() != QVariant::Type::ByteArray ||
        !opargs.at(1).canConvert<uint64_t>()) {
...
void Shell::handleGridLine(const QVariantList &opargs) {
    if (opargs.size() < 4 || !opargs.at(0).canConvert<qint64>() ||
        !opargs.at(1).canConvert<qint64>() ||
        !opargs.at(2).canConvert<qint64>() ||
        static_cast<QMetaType::Type>(opargs.at(3).type()) !=
            QMetaType::QVariantList) {
...
void Shell::handleGridCursorGoto(const QVariantList &opargs) {
    if (opargs.size() < 3 || !opargs.at(1).canConvert<quint64>() ||
...
void Shell::handleGridScroll(const QVariantList &opargs) {
    if (opargs.size() < 7 || !opargs.at(0).canConvert<uint64_t>() ||
        !opargs.at(1).canConvert<uint64_t>() ||
...
void Shell::handleGuiAdaptiveColor(const QVariantList &opargs) noexcept {
    if (opargs.size() < 2 || !opargs.at(1).canConvert<bool>()) {
...
void Shell::handleGuiAdaptiveStyle(const QVariantList &opargs) noexcept {
    if (opargs.size() < 2 || !opargs.at(1).canConvert<QByteArray>()) {
...
void Shell::neovimMouseEvent(QMouseEvent *ev) {
    ...
    if (ev->type() == QEvent::MouseMove) {
        ...
        inp = Input::convertMouse(ev->button(), ev->type(), ev->modifiers(),
                                  pos, m_mouseclick_count);
    }
    if (inp.isEmpty()) {
        return;
    }
...
void Shell::mouseReleaseEvent(QMouseEvent *ev) { neovimMouseEvent(ev); }
...
/*static*/ QString Shell::GetWheelEventStringAndSetScrollRemainder(
    const QWheelEvent &ev, QPoint &scrollRemainderOut, QSize cellSize,
    int deltasPerStep) noexcept {
    int invertConstant{(ev.inverted()) ? -1 : 1};

#ifdef Q_OS_MACOS
    auto naturalScrolling =
        CFPreferencesGetAppBooleanValue(CFSTR("com.apple.swipescrolldirection"),
                                        kCFPreferencesCurrentUser, NULL);
    if (naturalScrolling) {
        invertConstant *= -1;
    }
#endif

    QPoint scrollRemainderAndEvent{(ev.angleDelta() * invertConstant) +
                                   scrollRemainderOut};
    ...
    const bool isScrollDeltaOverflow{
...
// TODO Issue#751:  Remove Deprecated code, keep #else below
    ...
    QPoint evCellPos{evPos.x() / cellSize.width(),
                     evPos.y() / cellSize.height()};
    ...
    return wheelEventString.arg(Input::GetModifierPrefix(ev.modifiers()))
        .arg(evCellPos.x())
        .arg(evCellPos.y());
}
...
void Shell::updateWindowId() {
    if (m_attached &&
        ...
        m_nvim->api0()->vim_command(
            QString("let v:windowid = %1").arg(window_id).toLatin1());
        updateClientInfo();
    }
}
...
void Shell::updateClientInfo() {
    if (m_attached) {
        ...
        if (api4) {
            ...
            api4->nvim_set_client_info("nvim-qt", version, "ui", QVariantMap(),
                                       attrs);
        }
    }
}
...
void Shell::focusInEvent(QFocusEvent *ev) {
    if (m_attached) {
        // Issue #329: The <FocusGained> key no longer exists, use autocmd
        // instead.
        m_nvim->api0()->vim_command("if exists('#FocusGained') | doautocmd "
                                    "<nomodeline> FocusGained | endif");
    }
...
void Shell::focusOutEvent(QFocusEvent *ev) {
    if (m_attached) {
        // Issue #591: Option <nomodeline> prevents unwanted interaction,
        // consistent with nvim.
        m_nvim->api0()->vim_command("if exists('#FocusLost') | doautocmd "
                                    "<nomodeline> FocusLost | endif");
    }
...
void Shell::tooltip(const QString &text) {
    ...
    m_tooltip->setMinimumWidth(
        GetHorizontalAdvance(QFontMetrics{m_tooltip->font()}, text));
    m_tooltip->setMaximumWidth(
        GetHorizontalAdvance(QFontMetrics{m_tooltip->font()}, text));
    m_tooltip->update();
}
...
void Shell::bailoutIfinputBlocking() noexcept {
    ...
    auto sendCtrlC = [api2](quint32 msgid, quint64 f,
                            const QVariant &r) noexcept {
...
ShellRequestHandler::ShellRequestHandler(Shell *parent) : QObject(parent) {}
...
void ShellRequestHandler::handleRequest(MsgpackIODevice *dev, quint32 msgid,
    ...
    if (method == "Gui" && args.size() > 0) {
        ...
        if (ctx == "GetClipboard" && args.size() > 1) {
            ...
            if (reg_name != "*" && reg_name != "+") {
                dev->sendResponse(msgid, QString("Unknown register"),
                                  QVariant());
                return;
            }
            ...
            // Check nvim, ops.c/get_clipboard() - Expected to return a list
            // with two items [register data, selection type]. The type can be
            // ommited.
            QVariantList result;
            ...
            if (clipboard_data->hasFormat(SELECTION_MIME_TYPE)) {
                QString type;
                QDataStream serialize(
                    clipboard_data->data(SELECTION_MIME_TYPE));
                serialize >> type;
                result.append(type);
            } else {
                result.append("");
...
void Shell::handleGetBackgroundOption(quint32 msgid, quint64 fun,
                                      const QVariant &val) {
